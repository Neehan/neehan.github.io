...
4. Under pre-layer normalization, token representations are roughly white across features. Consequently, the scaled attention logits $QK^T/\sqrt{d}$ approximates the pairwise **directional similarity** between tokens. If $Q=K$, this is a covariance matrix.
